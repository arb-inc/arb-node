...
| ------------------- | ------------------------ | --------------- |
| `x-arb-inc-api-key` | `apiKey` | bearerAuth |
...
init({ server: myServer, endpoints: [createCases, myCustomEndpoint] });
```
...
- `list_schemas` (`write`): Retrieves all available schemas for case phases and documents.
